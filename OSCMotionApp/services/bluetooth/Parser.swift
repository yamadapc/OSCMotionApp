...
func hexadecimalRepresentation(of byteArray: [UInt8]) -> String {
...
class Parser {
  ...
  private func processReceivedData(dataBytes: [UInt8]) -> [SensorDataPacket] {
    ...
    while dataBuffer.count >= 11 {
      let packet = Array(dataBuffer[0..<11])
      ...
      case ANGLE:
        ...
        let temperature = Double(Int16(packet[9]) << 8 | Int16(packet[8])) / 340.0 + 36.25
        let data = SensorDataPacket.angle(x: angleX, y: angleY, z: angleZ, temperature: temperature)
        processedData.append(data)
      default:
        break
      }
      ...
      // Parser.logger.debug("Packet received \(packetStr)")
      dataBuffer = Array(dataBuffer.dropFirst(packet.count))
    }
